...
Pylint\cite{Pylint} es un programa de código abierto\footnote{\url{https://github.com/PyCQA/pylint}} que analiza código Python. Nos ofrece información sobre cómo está escrito el código analizado, según las normas de estilo PEP-8, y clasifica los errores en 5 tipos, a saber:
\begin{itemize}
...
SQLite\cite{Sqlite} es un sistema de gestión de Bases de datos relacional, compatible con ACID (Atomicidad, Consistencia, Aislamiento, Durabilidad), escrita en C. Es de dominio público. \\
...
\end{figure}
Es la última versión de HTML\cite{HTML5} (HyperText Markup Language). El término representa dos conceptos diferentes:
\begin{itemize}
...
JavaScript\cite{HTML5} es un lenguaje de programación que se utiliza principalmente para crear páginas web dinámicas.
...
Bootstrap\cite{Bootstrap} fue desarrollado por Mark Otto y Jacbod Thornton de Twitter, como un marco de trabajo (framework) para fomentar la consistencia a través de herramientas internas. Antes de Bootstrap, se usaban varias librerías para el desarrollo de interfaces de usuario, las cuales guiaban a inconsistencias y a una carga de trabajo alta en su mantenimiento.
...
\item \textbf{Analisis: } En este objeto guardamos la información de los errores de un repositorio completo. Sumando los errores de cada archivo, al tipo de error correspondiente. Así podemos analizar los datos rápidamente.
